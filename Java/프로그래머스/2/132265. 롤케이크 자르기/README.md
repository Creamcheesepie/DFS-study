...
한쪽 끝단에서 시작해서 서로 가지는 원소의 가짓수가 동일한 구간의 개수를 세면 된다.
예전엔 어떻게 풀어야하지? 한참 고민했던 기억이 나는데, 결국 무식하게 문제를 푸는게 제일 빠른 정답인 문제이다.
다만 Map 구현체를 쓰게 되면 해싱 -> 버킷 탐색의 문제가 있고, 래핑 반복에 의해 속도 저하가 있어서 이번엔 배열로 푼 풀이도 추가로 해 두었다.
